/**
 * @autor Gayani
 * @date 22-08-2014 Java script used to validate and functionality for customer
 *       details page
 */
...
function validatePayment() {
	...
	var isValid = true;
	// card details
	if ($.trim($('#cardHolderName').val()) == ""|| $.trim($('#cardHolderName').val()).length < 0) {
		setErrorPosition("Please enter Card Holder Name",$('#cardHolderName'), true, null);
		isValid = false;
	} else if (!$('#cardHolderName').val().match(namePattern)) {
	...
	if ($('#cardExpireMoth').val() == ""|| $('#cardExpireMoth').val().length < 0) {
	...
	if ($('#cardCW').val() == "" || $('#cardCW').val().length < 0) {
		setErrorPosition("Please enter cardCW .", $('#cardCW'), true, null);
		isValid = false;
	} else if ($('#cardCW').val().length < 3) {
...
function creditCardSelect() {
	...
	if ($('#cardType-C').attr('checked')) {
		if (($('#cardNumber_1').val().substring(0, 1) != "6")) {
			setErrorPosition("Please fill the credit card number in the format of 6XXX XXXX XXXX XXXX",$('#ccDiv'), false, null);
			isValid = false;
		}
	}
